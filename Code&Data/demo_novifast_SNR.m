...
for snr=1:numel(SNR90)
    for iter=0:itermax
        ...
        fprintf("Kgt Size where Mask==1: %d x %d\n", size(Kgt(mask==1)));
        ...
        if iter==0
            ...
            %[K, T1] =nelder_mead_optimization( im, alpha, TR, options);
            %[K, T1] =if_image( im, alpha, TR, options);
            %[K, T1] = model_based_optimization( im, alpha, TR, options);

            %[K, T1] = implicit_filtering_optimization( im, alpha, TR, options_imfi);
            ...
            %[K_if, T1_if] = novifast_image(im, alpha, TR, options_if);

        end
        %disp(size(mask)); % Print the entire mask array
        %disp(size(mask == 1)); % Print the result of the logical expression mask == 1
        fprintf("Size of T1gt: %d x %d\n", size(T1gt));
        fprintf("Size of T1: %d x %d\n", size(T1));
        fprintf("Result of Logical Indexing: %d x %d\n", size(mask == 1));
